...
def check_position_knowledge_questions(BC, given_char):
    ...
    if len(list_check_1) > 1 or len(list_check_2) > 1:
        ...
        if not re.match(r"\?[A-Z]*",BC[len(BC)-1]):#Questions
            ...
            sys.exit(0)
        elif not re.match(r"=[A-Z]*",BC[len(BC)-2]):#Knowledge
